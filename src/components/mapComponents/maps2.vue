  ...
  methods: {
    init () {
      ...
      let info = {
        ...
        zoom: 12,   //设置地图缩放级别
        mapStyleId: 'style' + this.styleNo.toString()//个性化地图样式设置1'//个性化地图样式设置
      }
    ...
    setPoints (data) {
      ...
      let _this = this
      this.dataPoints.forEach(function (item, index) {
        if (index < _this.dataPoints.length / 4) {
          item.styleId = 'greenStyleColor';
        } else if (index < (_this.dataPoints.length * 2 / 4)) {
          item.styleId = 'yellowStyleColor';
        } else if (index < (_this.dataPoints.length * 3 / 4)) {
